...
@Service
@Slf4j
public class CASCredentialRetrieverService {

	public SympaCredential getCredentialForService(String targetService) {

    log.info("ask proxy ticket for {}", targetService);
    CasAuthenticationToken token = (CasAuthenticationToken) SecurityContextHolder
      .getContext()
      .getAuthentication();
    String password =  token.getAssertion().getPrincipal().getProxyTicketFor(targetService);
